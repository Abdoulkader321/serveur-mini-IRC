...
enum AskRessource {
    ...
    UserSubscribedToChannel(String, String, oneshot::Sender<bool>),
    UserDisconnected(String),
    TransferMessageToChannel(String, String, String)
}
...
async fn server_database(rx: &mut Receiver<AskRessource>) {
    ...
    loop {
        match rx.recv().await {
            ...
            Some(AskRessource::UserSubscribedToChannel(username, channel_name, resp)) => {
                ...
                if let Some(index) = channel_index {
                    ...
                    let user_index = channels[index].sender.into_subscribers().iter().position(|name| *name == username);
            ...
            Some(AskRessource::TransferMessageToChannel(username, channel_name, content)) => {

                let index = channels.iter().position(|channel| channel.name == channel_name).unwrap();
        ...
        match typed_reader.recv().await {
            ...
            Ok(Some(Request::Message { to, content })) => match to {
                MessageReceiver::Channel(channel_name) => {
                    let (tx, rx) = oneshot::channel();
                    let ressource =
                        AskRessource::UserSubscribedToChannel(username.clone(), channel_name.clone(), tx);
                    ...
                    match thread_tx.send(ressource).await {
                        Ok(_) => match rx.await {
                            ...
                            Ok(true) => {
                                let ressource = AskRessource::TransferMessageToChannel(username.clone(), channel_name, content);

                                if thread_tx.send(ressource).await.is_err(){
