...
fn check_channel_exists(channels: &Vec<Channel>, channel_name: String) -> Option<usize> {
    channels
        .iter()
            ...
            Some(AskRessource::AskToLeaveChannel(username, channel_name, resp)) => {
                ...
                if let Some(index) = channel_index {
                    if check_user_in_channel(&channels, index, username.clone()).is_some() {
                        ...
                        let ressource = BroadcastMessage::Leave(username, channel_name);
...
async fn handle_waiting_for_messages(
    ...
    while !finished {
        match broadcast_receiver.recv().await {
            ...
            Ok(BroadcastMessage::Leave(user_name, channel)) => {
                ...
                if user_name == username {
                    ...
                    respond_to_client(writer_tx, Response::Channel { op: chan_op, chan: channel }).await;
                }                
