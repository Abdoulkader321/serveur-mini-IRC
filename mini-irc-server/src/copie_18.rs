    ...
    loop {
        match rx.recv().await {
            ...
            Some(AskRessource::UserDisconnected(username)) => {
                let index = connected_clients.iter().position(|name| *name == username).unwrap();
                ...
                match thread_tx.send(ressource).await {
                    Ok(_) => match rx.await {
                        Ok(mut broadcast_receiver) => {
                            ...
                            .await;
                            handle_waiting_for_messages(&mut broadcast_receiver, writer).await;
                        }
                        Err(e) => {
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    loop {
        ...
        tokio::spawn(async move {
            ...
            if let Some(username) = is_client_accepted(&mut reader, &mut writer, &mut thread_tx).await {
